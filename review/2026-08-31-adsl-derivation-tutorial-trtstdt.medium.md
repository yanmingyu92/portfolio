# Medium upload-ready draft — ADSL Derivation Walkthrough: TRT01SDT, Flags, and the Merge Traps
...
2. Title: ADSL Derivation Walkthrough: TRT01SDT, Flags, and the Merge Traps
...
4. Set the canonical link (SEO): Story settings -> Advanced settings ->
   "This story was originally published elsewhere" -> https://jaimeyan.com/blog/adsl-derivation-tutorial-trtstdt.html
5. Tags (Medium allows 5): clinical-sas, adam, adsl, cdisc
6. Preview, then Publish.

## Alternative: importer (fast, but no table fidelity)

- https://medium.com/p/import with https://jaimeyan.com/blog/adsl-derivation-tutorial-trtstdt.html — sets canonical
  automatically, but Medium flattens HTML tables; prefer the paste path above
  for this series.

---

Canonical: https://jaimeyan.com/blog/adsl-derivation-tutorial-trtstdt.html
Video: https://youtu.be/CkOl-p3lQRM
...
The same derivations exist as admiral function calls in R, derive_vars_merged for the earliest-dose merge and flag functions for the populations, and the pharmaverse admiralpython package carries the pattern to Python shops. The intent reads more cleanly than stacked data-step merges, which is exactly what my fine-tuning experiments exploited: [fine-tuning a small LLM on admiral code](https://jaimeyan.com/blog/fine-tuning-small-llms-admiral-r.html) worked because one-variable-at-a-time derivations map onto a question-answer format a model can learn.
...
So the verification habit: pick three random subjects, ask for the full derivation trace of their TRT01SDT, and hand-check against the SDTM records. An agent that invented a fallback fails exactly here. The accountability boundary does not move: the validation signature belongs to the programmer, not the tool, which is the core argument of [why LLM agents fail in regulated programming](https://jaimeyan.com/blog/why-llm-agents-fail-regulated-programming.html).

> The agentic way — Agents draft ADSL merges and cascades in minutes, but they invent SAP fallback rules and partial-date conventions that read plausibly; the failure mode is a clean log over an unsourced decision.
>
> Before trusting agent-drafted ADSL code, demand the derivation trace for three random subjects and the SAP citation behind every fallback step.
>
...
That closes the ADSL walkthrough. Next in the bootcamp: [Part 3, programming TLFs from shell to RTF](https://jaimeyan.com/blog/tlf-shell-to-rtf-tutorial.html). If you arrived from [Part 1, the SCE guide](https://jaimeyan.com/blog/sce-statistical-computing-environment-guide.html), the ADaM line continues later with BDS structure in ADLB and ADVS, OCCDS and ADAE, and ADTTE. The [series roadmap](https://jaimeyan.com/blog/clinical-sp-bootcamp-roadmap.html) tracks everything published.
