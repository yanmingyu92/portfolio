# Medium upload-ready draft — SDTM AE Domain Mapping: A Worked Example from Raw EDC Data
...
2. Title: SDTM AE Domain Mapping: A Worked Example from Raw EDC Data
...
4. Set the canonical link (SEO): Story settings -> Advanced settings ->
   "This story was originally published elsewhere" -> https://jaimeyan.com/blog/sdtm-ae-domain-mapping-example.html
5. Tags (Medium allows 5): clinical-sas, sdtm, adverse-events, meddra
6. Preview, then Publish.

## Alternative: importer (fast, but no table fidelity)

- https://medium.com/p/import with https://jaimeyan.com/blog/sdtm-ae-domain-mapping-example.html — sets canonical
  automatically, but Medium flattens HTML tables; prefer the paste path above
  for this series.

---

Canonical: https://jaimeyan.com/blog/sdtm-ae-domain-mapping-example.html
...
**Validation in CI, per data drop.** CORE runs against every rebuilt domain, and the triage table above lives as a versioned log next to the code, reviewer-accessible and diff-able between cuts. The economics of free, scriptable validation are covered in [CDISC CORE Explained](https://jaimeyan.com/blog/cdisc-core-validation-explained.html); the operational change is that validation becomes a build step, not an event.
...
**The spec rules online.** Decision rows live in the shared mapping specification (the [sdtm-mapping-conventions](https://jaimeyan.com/skills/sdtm-mapping-conventions/SKILL.md) skill distills this discipline into a drop-in checklist), and the AE program reads as a direct transcription of those rows.
...
> The agentic way — Agents draft a full AE mapping in minutes from spec plus raw extract; the failure mode is helpful invention of MedDRA terms and quiet completion of partial dates. Accept coded terms only when joined from the real coding file, and diff every date string against collection.
>
...
Previous in the series: [SDTM domain basics](https://jaimeyan.com/blog/sdtm-tutorial-domain-basics.html). Next: [How to write an SDTM mapping specification](https://jaimeyan.com/blog/sdtm-mapping-spec-walkthrough.html). For the validation engine behind the triage table (public YAML rules, free execution, and its structural limits), read [CDISC CORE Explained](https://jaimeyan.com/blog/cdisc-core-validation-explained.html).
