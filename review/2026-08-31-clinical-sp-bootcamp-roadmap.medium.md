# Medium upload-ready draft — The Clinical SP Bootcamp: A Learning Path from Raw Data to TLF
...
2. Title: The Clinical SP Bootcamp: A Learning Path from Raw Data to TLF
...
4. Set the canonical link (SEO): Story settings -> Advanced settings ->
   "This story was originally published elsewhere" -> https://jaimeyan.com/blog/clinical-sp-bootcamp-roadmap.html
5. Tags (Medium allows 5): clinical-sas, career, cdisc, learning-path
6. Preview, then Publish.

## Alternative: importer (fast, but no table fidelity)

- https://medium.com/p/import with https://jaimeyan.com/blog/clinical-sp-bootcamp-roadmap.html — sets canonical
  automatically, but Medium flattens HTML tables; prefer the paste path above
  for this series.

---

Canonical: https://jaimeyan.com/blog/clinical-sp-bootcamp-roadmap.html
Video: https://youtu.be/fQjerkdRGJQ
...
-----------------------------------------------------------------------------------------------------+--------------+---------------------------------------------------------------------------------------------------
[1 — The statistical computing environment](https://jaimeyan.com/blog/sce-statistical-computing-environment-guide.html)  |  SCE         |  Explain what an SCE actually enforces and stand up a study workspace that runs the same way twice
[2 — ADSL derivations: TRT01SDT](https://jaimeyan.com/blog/adsl-derivation-tutorial-trtstdt.html)                        |  ADaM        |  Turn a SAP rule into a defended derivation with documented fallback behavior
[3 — TLF programming: shell to RTF](https://jaimeyan.com/blog/tlf-shell-to-rtf-tutorial.html)                            |  SDTM/TLF    |  Take a static mock shell and ship the table behind it as a reviewable RTF
[4 — SDTM domain basics](https://jaimeyan.com/blog/sdtm-tutorial-domain-basics.html)                                     |  SDTM/TLF    |  Explain what a domain is and map one raw lab file into standard structure
[5 — SDTM AE domain mapping](https://jaimeyan.com/blog/sdtm-ae-domain-mapping-example.html)                              |  SDTM/TLF    |  Build the AE domain end to end: MedDRA coding, flags, dates, and --SEQ
[6 — SDTM mapping specifications](https://jaimeyan.com/blog/sdtm-mapping-spec-walkthrough.html)                          |  SDTM/TLF    |  Read and write the spec that lets someone else reproduce your mapping
[7 — ADaM BDS: ADLB and ADVS](https://jaimeyan.com/blog/adam-bds-adlb-advs-tutorial.html)                                |  ADaM        |  Structure BDS datasets with parameters, baselines, and analysis visits
[8 — ADaM OCCDS: ADAE](https://jaimeyan.com/blog/adam-occds-adae-tutorial.html)                                          |  ADaM        |  Build occurrence datasets that answer who had what, and when
9 — ADaM ADTTE: time to event                                                                        |  ADaM        |  Derive a time-to-event dataset that survives a statistician's questions
[10 — Clinical SAS interview questions](https://jaimeyan.com/blog/clinical-sas-interview-questions-guide.html)           |  Career      |  Walk into the interview knowing the four rounds and the signals they grade
...
> The agentic way — Assistants now draft competent SAS and R for CDISC tasks on demand, which moves a learner's value from writing programs to specifying and verifying them; the failure mode is shortcutting the reading fluency you will later be paid to check.
>
...
Start where your syllabus says. New to CDISC means Part 4 — SDTM domain basics; setting up a study environment first means [Part 1 — the statistical computing environment guide](https://jaimeyan.com/blog/sce-statistical-computing-environment-guide.html). For which AI uses carry evidence rather than demos, read the [state-of-field survey](https://jaimeyan.com/blog/llm-clinical-statistical-programming-state-2026.html).
