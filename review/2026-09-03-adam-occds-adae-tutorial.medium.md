# Medium upload-ready draft — ADaM OCCDS Explained: Programming ADAE with Treatment Flags
...
2. Title: ADaM OCCDS Explained: Programming ADAE with Treatment Flags
...
4. Set the canonical link (SEO): Story settings -> Advanced settings ->
   "This story was originally published elsewhere" -> https://jaimeyan.com/blog/adam-occds-adae-tutorial.html
5. Tags (Medium allows 5): clinical-sas, adam, occds, adae, safety
6. Preview, then Publish.

## Alternative: importer (fast, but no table fidelity)

- https://medium.com/p/import with https://jaimeyan.com/blog/adam-occds-adae-tutorial.html — sets canonical
  automatically, but Medium flattens HTML tables; prefer the paste path above
  for this series.

---

Canonical: https://jaimeyan.com/blog/adam-occds-adae-tutorial.html
Video: https://youtu.be/RMOF0kWNe_8
...
The verification habit mirrors the one from the BDS post: the agent states its rule and the rule's source before anyone reads the code. Then hand-check three borderline subjects: one pre-dose event, one month-only onset, one event dated the same day as first dose. The accountability boundary stays put: whoever signs the QC document owns the flag logic, agent-drafted or not. The general argument is in [why LLM agents fail in regulated programming](https://jaimeyan.com/blog/why-llm-agents-fail-regulated-programming.html).

> The agentic way — Agents draft the AE-to-ADSL merge and treatment-emergent logic quickly, but they invent emergence conventions and partial-date defaults; the failure mode is a defensible-looking flag with no SAP citation behind it.
>
> Before trusting agent-drafted ADAE code, have it quote the emergence rule it implemented, then check three borderline subjects against source by hand.
>
...
Next in the bootcamp: Part 9, ADTTE and time-to-event programming. Earlier in the arc: [Part 7, BDS structure for ADLB and ADVS](https://jaimeyan.com/blog/adam-bds-adlb-advs-tutorial.html) and [Part 2, the ADSL walkthrough](https://jaimeyan.com/blog/adsl-derivation-tutorial-trtstdt.html). The [series roadmap](https://jaimeyan.com/blog/clinical-sp-bootcamp-roadmap.html) tracks what is published.
